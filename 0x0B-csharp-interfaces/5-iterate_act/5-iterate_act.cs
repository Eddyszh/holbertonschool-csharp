...
class RoomObjects
{
    ...
    /// <param name="type">type of the objects in the list</param>
    public static void IterateAction(List<Base> roomObjects, Type type)
    {
        foreach (var item in roomObjects)
        {
            ...
            {
                IInteractive obj = (IInteractive)item;
                obj.Interact();
            }
            if (item is IBreakable && type == typeof(IBreakable))
            {
                IBreakable obj = (IBreakable)item;
                obj.Break();
            }
            if (item is ICollectable && type == typeof(ICollectable))
            {
                ICollectable obj = (ICollectable)item;
                obj.Collect();
            }
        }
    }
